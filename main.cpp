...
{
    double real_num; double imag_num , tan_ang , tan_ratio , pos_angle;
    if(num == "-1")
    {
        mag = 1;
        ang = 180;
...
string omit_row(string mat , int r)
{
    int numrows = num_of_rows(mat);
    string res="[";
...
string minorss(string mat , int r , int c)
{
    string res="[";
    string newmat=omit_row(mat , r);
    int rowsnum=num_of_rows(newmat);

    for(int i=0 ; i<rowsnum ; i++)
    {
        A[i] = mat_row(newmat , i+1);
        A[i] = omit_element_fromRow(A[i] , c);
        res+=A[i]+";";
    }
...
string sign_rule(string mat)
{
    int n=num_of_rows(mat);
    string row="" ; string res="[";
    string x;
    for(int i=1;i<=n;i++)
    {
        if(i%2 != 0)
        {
            ...
            for(int j=1 ; j<=n ; j++)
            {
                if(j%2 != 0)
                {
                   x= eval(element_extract(mat,i,j) ,'x' , "1");
...
string minor_mat(string mat)
{
    ...
    res.erase(res.length()-1);
    res+="]";
    return sign_rule(res);
}

string adj(string mat)
{
    string minn=minor_mat(mat);
    string res=mat_trans(minn);
    return res;
}

string mat_inv(string mat)
{
    string d=det(mat);
    string c=eval("1" , '/' , d);
    string a=adj(mat);
    string res=numXmat(a , c);
    res = numXmat(res , "-1");
    return res;
}

string mat_div(string mat1 , string mat2)
{
    string newmat = mat_inv(mat2);
    string res = mat_multiply(mat1 , newmat);
    return res;
}
...
string eval(string num1 , char operator_ , string num2)
{
    ...
    case 'x':
        ...
        string res = comp_multiply(num1 , num2);
        if(num2.find("e") != -1)
        {
        int n=num2.length();
        string po="1";
        string p=num2.substr(num2.find("e")+1 , n-num2.find("e")-1 );
        int x=fabs(atof(p.c_str()));
        for(int i=1;i<=x;i++)
        {
            po+="0";
        }
        if(p[0] == '+')
        {
            res=comp_multiply(res , po);
        }
        else if(p[0] == '-')
        {
            res=comp_divide(res , po);
        }
        }
        return res;
        }
    ...
    case '/':
        {
        string res1 = comp_divide(num1 , num2);
        if(num2.find("e") != -1)
        {
        int n=num2.length();
        string po="1";
        ...
        if(p[0] == '+')
        {
            res1=comp_divide(res1 , po);
        }
        else if(p[0] == '-')
        {
            res1=comp_multiply(res1, po);
        }
        }
